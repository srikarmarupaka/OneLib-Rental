...
export const WishlistView: React.FC<WishlistViewProps> = ({ wishlist, onRemove, onMoveToCart, onBrowse }) => {
  if (wishlist.length === 0) {
    return (
      <div className="max-w-7xl mx-auto px-4 py-24 text-center">
        ...
        <p className="text-slate-500 mb-8">Save books you want to read later.</p>
        <button 
          onClick={onBrowse}
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      <h1 className="text-3xl font-serif font-bold text-slate-900 mb-8">My Wishlist</h1>
      
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
        {wishlist.map((book) => (
          <div key={book.id} className="flex gap-4 p-4 bg-white rounded-xl border border-slate-100 shadow-sm hover:shadow-md transition-shadow">
            <img src={book.coverUrl} alt={book.title} className="w-24 h-36 object-cover rounded-lg shadow-sm" />
            <div className="flex-1 flex flex-col justify-between py-1">
              <div>
                <h3 className="text-lg font-serif font-bold text-slate-900 line-clamp-2">{book.title}</h3>
                <p className="text-slate-500 text-sm mb-2">{book.author}</p>
                <div className="text-sm font-bold text-slate-700">₹{book.rentPrice} <span className="text-slate-400 font-normal">/ 14 days</span></div>
              </div>
