...
export const ReaderModal: React.FC<ReaderModalProps> = ({ book, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-[70] bg-white flex flex-col animate-in fade-in duration-200">
      {/* Reader Header */}
      <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100 bg-white shadow-sm">
        <div className="flex items-center gap-3">
           <button onClick={onClose} className="p-2 hover:bg-slate-100 rounded-full transition-colors">
             <X size={20} className="text-slate-600" />
           </button>
           <div>
             <h3 className="font-serif font-bold text-slate-800 text-sm sm:text-base line-clamp-1">{book.title}</h3>
        ...
        <div className="flex items-center gap-2">
           <button 
             ...
             className="p-2 hover:bg-slate-100 rounded-full text-slate-600 font-serif font-bold"
           >
             A-
           </button>
           <button 
      ...
      {/* Reader Content */}
      <div className="flex-1 overflow-y-auto bg-[#faf9f6] p-4 sm:p-8 flex justify-center">
         <div 
           className="max-w-2xl w-full bg-white shadow-sm border border-slate-100 min-h-full p-8 sm:p-12 leading-relaxed text-slate-800 font-serif transition-all"
         ...
         >
           ...
           <div className="mt-12 flex justify-center text-xs text-slate-400">
             Page {page} of 240
           </div>
         </div>
      </div>
      ...
      <div className="px-4 py-4 border-t border-slate-100 bg-white flex justify-center gap-8 items-center">
         <button 
           onClick={() => setPage(Math.max(1, page - 1))}
           disabled={page === 1}
           className="p-2 text-slate-600 hover:text-amber-600 disabled:text-slate-300 disabled:cursor-not-allowed"
         >
           <ChevronLeft size={24} />
         </button>
